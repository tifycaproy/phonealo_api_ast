...
class phpmailer
{
    ...
    function AddAddress($address, $name = "") {
	 ...
	 $this->to[$cur][1] = $name;
	  ...
	  /**
     * Esta función no estaba en la clase original y se ha implementado
     * por exigencias del guión.
     * Agrega una sola dirección de correo.
     */
    function AddUnaAddress($address, $name = "") {
    ...
    */
    ...
    function smtp_send($header, $body) {
        ...
        if(!$connection)
        {
            $this->error_handler("SMTP Error: No se pudo conectar con el servidor host SMTP");
            return false;
        }
	...
	if ($this->Helo != '')
        ...
        if($this->SMTPAuth)
        {
            ...
            {
                $this->error_handler("SMTP Error: No se pudo autentificar");
                return false;
            }
        }
        ...
        {
            $e = sprintf("SMTP Error: La dirección De: [%s] ha fallado", $smtp_from);
        ...
        if(count($bad_rcpt) > 0)
        {
            ...
            $e = sprintf("SMTP Error: El siguiente buzón ha fallado [%s]", $e);
            $this->error_handler($e);
        ...
        {
            $this->error_handler("SMTP Error: Datos no aceptados");
            return false;
        }
        $smtp->Quit();
        $smtp->Close();
    ...
    function AddAttachment($path, $name = "", $encoding = "base64", $type = "application/octet-stream") {
        if(!@is_file($path))
        {
            $this->error_handler(sprintf("No se pudo acceder al fichero [%s]", $path));
            return false;
        }
	...
	/**
	*Esta función AddUnoAttachment no forma parte de la clase en su origen si no que se ha reimplementado por
	*exigencias del guión. Esta función sólo permite añadir un fichero por envío.
	*/
	function AddUnoAttachment($path, $name = "", $encoding = "base64", $type = "application/octet-stream") {
        if(!@is_file($path))
        {
            $this->error_handler(sprintf("No se pudo acceder al fichero [%s]", $path));
            return false;
        ...
        // Append to $attachment array
        $this->attachment[0][0] = $path;
        $this->attachment[0][1] = $filename;
        $this->attachment[0][2] = $name;
        $this->attachment[0][3] = $encoding;
        $this->attachment[0][4] = $type;
        $this->attachment[0][5] = false; // isStringAttachment
        $this->attachment[0][6] = "attachment";
        $this->attachment[0][7] = 0;
